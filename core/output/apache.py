...
class Apache(Output):
    ...
    def write_apache_config(self, args, ips: List[str], hosts: List[str], agents: List[str]) -> None:
        #> ----------------------------------------------------------------------------
        ...
        self.add_comment(" Redirect a file/path to a target backend file/path\n")
        ...
        self.add_comment(" Redirect and proxy a file/path to another system's file/path\n")
    ...
    def write_ip_rules(self, ips: List[str], end=False) -> None:
        ...
        if end:
            for ip in ips[:-1]:
                self.write(f'RewriteCond\t\t\t\texpr\t\t\t\t\t"-R \'{ip}\'"\t[OR]\n')
            self.write(f'RewriteCond\t\t\t\texpr\t\t\t\t\t"-R \'{ips[-1]}\'"\n')
        else:
            for ip in ips:
                self.write(f'RewriteCond\t\t\t\texpr\t\t\t\t\t"-R \'{ip}\'"\t[OR]\n')
    ...
    def write_agent_rules(self, agents: List[str], end=False) -> None:
        ...
        if end:
            for agent in agents[:-1]:
                self.write(f'RewriteCond\t\t\t\t%{{HTTP_USER_AGENT}}\t\t\t\t\t{agent}\t[OR]\n')
            self.write(f'RewriteCond\t\t\t\t%{{HTTP_USER_AGENT}}\t\t\t\t\t{agents[-1]}\n')
        else:
            for agent in agents:
                self.write(f'RewriteCond\t\t\t\t%{{HTTP_USER_AGENT}}\t\t\t\t\t{agent}\t[OR]\n')
    ...
    def write_host_rules(self, hosts: List[str], end=False) -> None:
        ...
        if end:
            for host in hosts[:-1]:
                self.write(f'RewriteCond\t\t\t\t%{{HTTP_HOST}}\t\t\t\t\t{host}\t[OR]\n')
            self.write(f'RewriteCond\t\t\t\t%{{HTTP_HOST}}\t\t\t\t\t{hosts[-1]}\n')
        else:
            for host in hosts:
                self.write(f'RewriteCond\t\t\t\t%{{HTTP_HOST}}\t\t\t\t\t{host}\t[OR]\n')
